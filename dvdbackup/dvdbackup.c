...
int DVDDisplayInfo(dvd_reader_t* dvd, char* device) {
	...
	for (i=0; i < titles_info->number_of_titles ; i++ ) {
		if (titles_info->titles[i].title_set == titles_info->main_title_set) {
			...
			printf(_("\tThe main feature has %d angle\n"), titles_info->titles[i].angles);
			printf(_("\tThe main feature has %d audio track\n"), titles_info->titles[i].audio_tracks);
			printf(_("\tThe main feature has %d subpicture channel\n"), titles_info->titles[i].sub_pictures);
			chapters=0;
			channels=0;
			...
			printf(_("\tThe main feature has a maximum of %d chapter in one of its titles\n"), chapters);
			printf(_("\tThe main feature has a maximum of %d audio channel in one of its titles\n"), channels);
			break;
		}
	}
	...
	for (f=0; f < title_set_info->number_of_title_sets ; f++ ) {
		...
		for (i=0; i < titles_info->number_of_titles ; i++ ) {
			if (titles_info->titles[i].title_set == f + 1) {
				...
				printf(_("\t\tTitle set %d has %d angle\n"), f + 1, titles_info->titles[i].angles);
				printf(_("\t\tTitle set %d has %d audio track\n"), f + 1, titles_info->titles[i].audio_tracks);
				printf(_("\t\tTitle set %d has %d subpicture channel\n"), f + 1, titles_info->titles[i].sub_pictures);
				break;
			}
		}
