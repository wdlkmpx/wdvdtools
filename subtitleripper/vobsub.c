/*
 * This file is copied from the mplayer distribution
 *   http://mplayer.dev.hu/homepage/news.html 
 *   (original file vobsub.c)
 * 
 * Thanks to the original author (who ever he/she is).
 *
 *
 * History:
 * 2002-08-09
...
//#include "version.h"
#undef VERSION
#define VERSION "0.90pre6-2.95.3 (subtitleripper)"
...
//#include "mp_msg.h"
#define mp_msg(a,b,args...) fprintf(stderr,##args),putc('\n',stderr)
#define spudec_new_scaled_vobsub(a, b, c, d, e) (void*) printf("BUG:spudec_new_scaled_vobsub called\n")
...
static int vobsub_add_id(vobsub_t *vob, const char *id, size_t idlen, const unsigned int index)
{
...
static int vobsub_add_timestamp(vobsub_t *vob, off_t filepos, unsigned int ms)
{
...
static int vobsub_parse_one_line(vobsub_t *vob, FILE *fd)
{
    ssize_t line_size;
    int res = -1;
    do
...
void * vobsub_open(const char *const name,const char *const ifo,const int force,void** spu)
{
...
static void create_idx(vobsub_out_t *me, const unsigned int *palette, unsigned int orig_width, unsigned int orig_height)
{
    int i;
    fprintf(me->fidx,
